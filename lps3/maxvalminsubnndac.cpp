...
details find_maximum_subarray(vector<int> a,int low,int high)
{
    ...
    if((left_details.sum > right_details.sum) || 
       (left_details.sum == right_details.sum && (left_details.high - left_details.low) >= (right_details.high - right_details.low)))
    {
        if((left_details.sum > cross_details.sum) || 
           (left_details.sum == cross_details.sum && (left_details.high - left_details.low) >= (cross_details.high - cross_details.low)))
        {
            return left_details;
        }
        else
    ...
    {
        if((right_details.sum > cross_details.sum) || 
           (right_details.sum == cross_details.sum && (right_details.high - right_details.low) >= (cross_details.high - cross_details.low)))
        {
...
int main()
{
    ...
    details ans;
    freopen("ascending_500.txt", "r", stdin);
    clock_t start = clock();
    cin>>n;
    for(i=0;i<n;i++)
    {
